...
def test_exports_are_downloadable():
    import httpx

    r = httpx.get(f"{SMOKE_URL}/exports/parts/v1")
    assert r.status_code == 200
    assert r.text.startswith("partNo,partName,cat,measure,partStatus")

    r = httpx.get(f"{SMOKE_URL}/exports/suppliers/legacy")
    assert r.status_code == 200
    assert "supplier_name,supplier_code,country,email" in r.text

    r = httpx.get(f"{SMOKE_URL}/exports/change-orders")
    assert r.status_code == 200

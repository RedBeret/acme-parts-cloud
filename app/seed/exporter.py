...
def export_change_orders_xlsx(db) -> None:
    ...
    sample = SAMPLES_DIR / "change_orders_sample.xlsx"
    sample_wb = openpyxl.Workbook()
    sample_ws = sample_wb.active
    sample_ws.title = "Change Orders"
    for source_row in ws.iter_rows(max_row=min(52, ws.max_row), values_only=True):
        sample_ws.append(source_row)
    sample_ws.merge_cells("A1:G1")
    sample_wb.save(sample)

...
A self-contained FastAPI + PostgreSQL sandbox that ships 5,000+ parts, 400 suppliers, 20,000 change orders, and 30,000 purchase orders — all seeded with **deterministic, configurable messiness** that mirrors real enterprise data debt.
...
graph TD
    ...
    Seed --> Gen["generators.py\n5K parts · 400 suppliers\n20K COs · 30K POs"]
    Seed --> Manifest["mess_manifest.json\n(ground truth)"]
    Seed --> Exporter["exporter.py\nCSV · XLSX · legacy encodings"]
    API --> UI["Jinja2 + HTMX\nDashboard · Parts · Suppliers · COs"]
```
...
Open **http://localhost:8000/** — the seeder runs automatically on first boot.
...
# Start Postgres separately (Docker or local install)
...
**Outcome:** The engineer used `mess_manifest.json` to verify their cleaning pipeline caught 100% of the injected defects before promoting to production. Build time: 4 hours. Production incident avoided.
...
Same `SEED + MESSINESS` always produces byte-identical data — safe to use as a regression baseline.
